...
export async function activate(context: vscode.ExtensionContext) {
    ...
    const commands = [
        ...
        vscode.commands.registerCommand(
            "fuzor-ai-transformer.searchTransformers",
            async () => {
        ...
        vscode.commands.registerCommand(
            "fuzor-ai-transformer.viewTransformerLibrary",
            async () => {
        ...
        vscode.commands.registerCommand(
            "fuzor-ai-transformer.createFolder",
            async () => {
        ...
        vscode.commands.registerCommand(
            "fuzor-ai-transformer.deleteFolder",
            async (item: TransformerTreeItem) => {
        ...
        vscode.commands.registerCommand(
            "fuzor-ai-transformer.renameFolder",
            async (item: TransformerTreeItem) => {
                if (item?.fuzorItem.folder !== undefined || item?.fuzorItem.folder !== null) {
                    ...
                    if (newName) {
                        try {
                            await transformerManager.renameFolder(
                                item.fuzorItem.folder!.id, 
                                newName
                            );
                            transformersProvider.refresh();
                            ...
                            if (error instanceof TransformerExistsError) {
                                vscode.window.showErrorMessage(
                                    `A folder with name "${newName}" already exists`
                                );
                            } else {
                                vscode.window.showErrorMessage(

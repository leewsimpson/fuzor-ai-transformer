...
suite("executeTransformers", () => {
		...
		test("should throw error for invalid input path", async () => {
			...
			await assert.rejects(
				...
				(error: Error) => {
					assert.strictEqual(
						error.message,
						'Validation failed: Input "input1" path does not exist or is invalid: invalid-path',
					)
					return true
				},
			)
		...
		test("should throw error for invalid output folder path", async () => {
			...
			await assert.rejects(
				...
				(error: Error) => {
					assert.strictEqual(
						error.message,
						"Validation failed: Output folder path is invalid or does not exist: invalid-folder",
					)
					return true
				},
			)
	...
	suite("input handling", () => {
		...
		test("should handle multiple file inputs", async () => {
			...
			sandbox
				.stub(require("fs"), "existsSync")
				.withArgs("valid-file1.txt")
				.returns(true)
				.withArgs("valid-file2.txt")
				.returns(true)
				.withArgs("valid-folder")
				.returns(true)
			sandbox.stub(require("fs"), "accessSync").returns(undefined)

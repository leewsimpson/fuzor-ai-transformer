...
suite('TransformerManager Behavior Tests', () => {
  ...
  suite('Additional Validation Tests', () => {
    ...
    test('should validate prompt placeholders', async () => {
      ...
      await assert.rejects(
        () => manager.createTransformer(noPlaceholder),
        TransformerValidationError
      );
      ...
      await assert.rejects(
        () => manager.createTransformer(invalidName),
        TransformerValidationError
      );
      ...
      await assert.rejects(
        () => manager.createTransformer(invalidType),
        TransformerValidationError
      );
      ...
      await assert.rejects(
        () => manager.createTransformer(duplicateNames),
        TransformerValidationError
      );
      ...
      await assert.rejects(
        () => manager.createTransformer(multipleFolders),
        TransformerValidationError
      );
    });
    ...
    test('should update input configuration based on prompt placeholders', async () => {
      ...
      assert.deepStrictEqual(updated.input.map(i => i.name), ['test1', 'test2', 'test3']);
      assert.deepStrictEqual(updated.input.map(i => i.type), ['text', 'file', 'folder']);
      assert.deepStrictEqual(updated.input.map(i => i.description), [
        'Input for test1',
        'Input for test2',
        'Input for test3'
      ]);
      assert.deepStrictEqual(updated.input.map(i => i.required), [true, true, true]);
    });
  ...
  suite('Folder Operations', () => {
    ...
    test('should validate folder configuration', async () => {
      ...
      await assert.rejects(
        () => manager.createFolder(invalidFolder),
        TransformerValidationError
      );
    });
  });
  ...
  suite('Execution Operations', () => {
    ...
    test('should validate transformer before execution', async () => {
      const invalidConfig = { ...baseConfig, prompt: '' };
      await assert.rejects(
        () => manager.executeTransformer(invalidConfig),
        TransformerValidationError
      );
    });

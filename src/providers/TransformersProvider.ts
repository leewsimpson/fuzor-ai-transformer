...
export class TransformersProvider implements vscode.TreeDataProvider<TransformerTreeItem>, vscode.TreeDragAndDropController<TransformerTreeItem> {
    ...
    public async handleDrag(source: TransformerTreeItem[], dataTransfer: vscode.DataTransfer, token: vscode.CancellationToken): 
    Promise<void> {
            dataTransfer.set('application/vnd.code.tree.treeTransformer', new vscode.DataTransferItem(source));
    }

    public async handleDrop(target: TransformerTreeItem | undefined, dataTransfer: vscode.DataTransfer, token: vscode.CancellationToken): Promise<void> {
    ...
    getChildren(element?: TransformerTreeItem): Thenable<TransformerTreeItem[]> {
        ...
        for (const item of allItems) {
            const parentId = item.fuzorItem.type === 'transformer' 

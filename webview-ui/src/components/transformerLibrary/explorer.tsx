...
const Folder: React.FC<{
	...
	onNameClick: (transfomerLibrarySelected: TransformerLibrary) => void
	isSelected: boolean
}> = ({ name, children, library, onNameClick, isSelected }) => {
...
const Explorer: React.FC<{
	library: TransformerLibrary
	onFolderClick: (transfomerLibrarySelected: TransformerLibrary) => void
}> = ({ library, onFolderClick }) => {
	...
	const renderItem = (key: string, value: any) => {
		if (typeof value === "object" && value !== null && !("name" in value)) {
			...
			const hasSubitems = Object.keys(value).some(
				(subKey) => typeof value[subKey] === "object" && value[subKey] !== null && !("name" in value[subKey]),
			)
			if (hasSubitems) {
				return (
					<Folder
						...
						name={key}
						library={value}
						onNameClick={handleFolderClick}
						isSelected={selectedLibrary === value}>
	...
	return <ul className="space-y-2">{entries.map(([key, value]) => renderItem(key, value))}</ul>
}
